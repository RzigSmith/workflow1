  ...
  async search(q) {
    ...
    container.innerHTML = results.map(u => {
      ...
      if (u.relation === 'accepted') {
        ...
        actionBtn = `<span style="font-size:0.75rem;color:var(--text-muted);">Demande envoyée</span>`;
...
const Social = {
  ...
  openChat(userId, name) {
    ...
    setTimeout(() => Chat.openWithUser(userId, name), 200);
  },
  ...
  async submitPost() {
    ...
    if (res.success) {
      ...
      WorkFlow.toast.show('Erreur lors de la publication', 'error');
    }
  },
...
const Feed = {
  ...
  renderPost(p) {
    ...
    const isMe = parseInt(p.id_user) === window.APP?.userId;
...
const Chat = {
  ...
  async loadConversations() {
    ...
    if (!Array.isArray(convs) || convs.length === 0) {
      list.innerHTML = `<div style="padding:1.25rem;text-align:center;color:var(--text-muted);font-size:0.8rem;">
        ...
        Aucune conversation<br>Commencez depuis vos <b>Amis</b>
      </div>`;
      return;
    }
    ...
    list.innerHTML = convs.map(c => {
      ...
      return `<div class="conv-item ${this.activeConvId == c.id_conversation ? 'active' : ''}"
                   onclick="Chat.openConv(${c.id_conversation}, '${escAttr(name)}')">
        <div class="user-avatar-sm" style="width:36px;height:36px;">${initials}</div>
        <div class="conv-meta">
  ...
  async openConv(convId, name) {
    ...
    items.forEach(i => { if (i.onclick && i.onclick.toString().includes(convId)) i.classList.add('active'); });
  ...
  async loadMessages(initial = false) {
    ...
    const params = initial
      ? { conv_id: this.activeConvId }
      : { conv_id: this.activeConvId, last_id: this.lastMsgId };
    const msgs = await API.get('get-messages', params);
    if (!Array.isArray(msgs) || msgs.length === 0) return;

    const container = document.getElementById('chat-messages');
    const myId = window.APP?.userId;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const observer = new MutationObserver(() => {
    const feedSection = document.querySelector('.page-section[data-id="messages"]');
    if (feedSection && !feedSection.classList.contains('active')) {
      Chat.stopPolling();
    }
  });
  const feedSection = document.querySelector('.page-section[data-id="messages"]');
  if (feedSection) observer.observe(feedSection, { attributes: true, attributeFilter: ['class'] });
});
